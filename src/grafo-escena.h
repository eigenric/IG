...
class RejillaParalelepipedo: public NodoGrafoEscena
{
   protected:
      unsigned int num_parametros = 1;
      mat4x4* pm_rotacion_paralelepipedo = nullptr;

   public:
      RejillaParalelepipedo();
      unsigned int leerNumParametros() const;
      void actualizarEstadoParametro(const unsigned iParam, const float);
      void fijarRotacion(const float angulo_rot);
};
